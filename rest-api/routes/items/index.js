...
app.get("/items/get-delete-item-page-data", authUser, async (req, res) => {
  try {
    ...
    const response = await api.getDeleteItemPageData(req.query.id, res.locals);
    res.json(response);
  } catch (error) {
    if (!(error instanceof Error)) {
      error.authUser = res.locals;
...
app.get("/items/get-ranked-items-by-page", authUser, async (req, res) => {
  try {
    ...
    const response = await api.getRankedItemsByPage(req.query.page, res.locals);
...
app.get("/items/get-newest-items-by-page", authUser, async (req, res) => {
  try {
    ...
    const response = await api.getNewestItemsByPage(req.query.page, res.locals);
