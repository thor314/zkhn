...
module.exports = {
  ...
  getItemById: async (itemId, page, authUser) => {
    ...
    /// IF USER SIGNED IN
    const [voteDoc, favoriteDoc, hiddenDoc, commentVoteDocs] =
      await Promise.all([
        ...
        for (let i = 0; i < commentVoteDocs.length; i++) {
          if (comment.id === commentVoteDocs[i].id) {
            comment.unvoteExpired =
              commentVoteDocs[i].date + 3600 * config.hrsUntilUnvoteExpires <
              moment().unix()
                ? true
                : false;
  ...
  getUserFavoritedItemsByPage: async (username, page, authUser) => {
    ...
    const [userFavoriteItemsDocs, totalFavoriteItemsCount] = await Promise.all([
  ...
  getUserUpvotedItemsByPage: async (page, authUser) => {
    ...
    for (i = 0; i < items.length; i++) {
      ...
      items[i].votedOnByUser = true;
      items[i].unvoteExpired =
        voteDocs[i].date + 3600 * config.hrsUntilUnvoteExpires < moment().unix()
          ? true
          : false;
    }
