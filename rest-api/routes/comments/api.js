...
module.exports = {
  ...
  getCommentById: async (commentId, page, authUser) => {
    ...
    const [commentVoteDoc, commentFavoriteDoc, commentVotesByUserDocs] =
      await Promise.all([
    ...
    const updateComment = (parentComment) => {
      if (parentComment.by === authUser.username) {
        const hasEditAndDeleteExpired =
          parentComment.created + 3600 * config.hrsUntilEditAndDeleteExpires <
      ...
      if (userCommentVotes.includes(parentComment.id)) {
        ...
        for (let i = 0; i < commentVotesByUserDocs.length; i++) {
          ...
          if (parentComment.id === commentVotesByUserDocs[i].id) {
            parentComment.upvotedByUser = commentVotesByUserDocs[i].upvote;
            parentComment.downvotedByUser = commentVotesByUserDocs[i].downvote;
          }
        }
      }
  ...
  getEditCommentPageData: async (commentId, authUser) => {
    const comment = await CommentModel.findOne({ id: commentId }).lean().exec();
  ...
  getNewestCommentsByPage: async (page, authUser) => {
    ...
    for (let i = 0; i < comments.length; i++) {
      ...
      if (comments[i].by === authUser.username) {
        const hasEditAndDeleteExpired =
          comments[i].created + 3600 * config.hrsUntilEditAndDeleteExpires <
  ...
  getUserFavoritedCommentsByPage: async (userId, page, authUser) => {
    ...
    const [userFavoriteCommentsDocs, totalFavoriteCommentsCount] =
      await Promise.all([
    ...
    for (let i = 0; i < userCommentVoteDocs.length; i++) {
      ...
      if (commentObj) {
        commentObj.upvotedByUser = userCommentVoteDocs[i].upvote || false;
        commentObj.downvotedByUser = userCommentVoteDocs[i].downvote || false;
        commentObj.votedOnByUser = true;
        commentObj.unvoteExpired =
          userCommentVoteDocs[i].date + 3600 * config.hrsUntilUnvoteExpires <
          moment().unix()
            ? true
            : false;
      }
