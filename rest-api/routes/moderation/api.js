...
module.exports = {
  ...
  getBannedUsersByPage: async (page) => {
    ...
    return {
      ...
      isMore:
        totalNumOfUsers >
        (page - 1) * config.bannedUsersPerPage + config.bannedUsersPerPage
          ? true
          : false,
    };
  },
  ...
  getModerationLogsByPage: async (category, page) => {
    ...
    if (category === "users") {
      ...
      dbQuery = {
        $or: [{ actionType: "kill-item" }, { actionType: "unkill-item" }],
      };
    } else if (category === "comments") {
      categoryString = "comments";
      dbQuery = {
        $or: [{ actionType: "kill-comment" }, { actionType: "unkill-comment" }],
      };
    } else {
      categoryString = "all";
      dbQuery = {};
    ...
    return {
      ...
      isMore:
        totalNumOfLogs >
        (page - 1) * config.moderationLogsPerPage + config.moderationLogsPerPage
          ? true
          : false,
    };
  },
